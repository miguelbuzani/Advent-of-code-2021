...
for line in lines:
    ...
    if cordX1 == cordX2:
        ...
        slope = (atoi(cordY2) - atoi(cordY1)) / (atoi(cordX2) - atoi(cordX1))

        if ((atoi(cordY2) - atoi(cordY1)) < 0) and (
            (atoi(cordX2) - atoi(cordX1)) < 0):
            slope = slope * -1
        ...
        if ((atoi(cordX2) - atoi(cordX1)) != 0):
            ...
            if slope > 0:
                tempX = atoi(cordX1)
                tempY = atoi(cordY1)
                numCordY2 = atoi(cordY2)
                numCordX2 = atoi(cordX2)
                # # print("tempX: %s tempY: %s", cordX2, cordY2)

                while tempX >= 0 and tempY >= 0:
                    # print("cordX1: %s cordY1: %s", cordX1, cordY1)

                    # print("tempX: %s tempY: %s", tempX, tempY)
                    # print("cordX: %s cordY: %s", cordX2, cordY2)
                    floorMap[tempY][tempX] = floorMap[tempY][tempX] + 1

                    if tempY == numCordY2 and tempX == numCordX2:
                        break
                    tempY += 1
                    tempX += 1
                    # print("tempX: %s tempY: %s", tempX, tempY)
